...
fn update_all_workspaces() -> eyre::Result<()> {
    println!("Updating all workspaces...");
    ...
    if !workspaces_dir.exists() {
        println!("No workspaces directory found. Nothing to update.");
        return Ok(());
    }
        ...
        .wrap_err("Failed to read workspaces directory")?
    {
        ...
        if let Some(workspace_name) = workspace_name {
            ...
            eprintln!("Skipping invalid workspace file: {}", path.display());
            continue;
        }
    }
...
fn update_workspace_internal(mut workspace: Workspace) -> eyre::Result<(usize, usize)> {
...
fn update_current_project() -> eyre::Result<()> {
    let project =
